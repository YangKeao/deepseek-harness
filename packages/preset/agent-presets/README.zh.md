...
随附 Web 的 `standard`、`ptc` 与 `cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)。`minimal` preset 保留固定的双工具训练配置。每个挂载压缩的 preset 同时把摘要路由固定为 `deepseek-official`/`deepseek-flash`（[原因](../../../.agents/notes/implemented/bug-fix/2026-09-20-saturated-provider-retry-and-pinned-summary-route.zh.md)）：摘要会重放会话的系统提示词、工具与被遮蔽消息，因此它需要的上下文余量多于那个已经溢出的会话；而继承一条更小的会话路由无法摘要那次溢出。
