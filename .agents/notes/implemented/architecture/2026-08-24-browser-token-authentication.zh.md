...
本决策定义默认的 `authentication: browser-token` 模式。显式启用的[外部访问控制模式](../feature/2026-09-17-connection-no-auth.zh.md)部分取代无条件认证要求；选择 browser-token 模式时，令牌与 cookie 的行为保持不变。

browser-token 模式下，`dsh-client-connection` 在分发前认证完整 Host API。每个 API Proxy 方法、Remote 一元调用、通用 Connection channel 和 Remote WebSocket stream 都要求同一个浏览器会话；endpoint 所有权与方法名称不改变 authority。既有 Host/Origin 校验先执行，继续负责 DNS rebinding 和跨站请求防御，失败时返回 403。Host 可信但没有有效浏览器会话时返回 401。浏览器信任规则仍由[载体级浏览器信任决策](2026-07-28-api-browser-trust-boundary.zh.md)持有。
