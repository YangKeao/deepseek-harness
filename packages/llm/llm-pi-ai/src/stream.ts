...
function classifyPiAiError(message: string): string {
  ...
  if (/\b(?:network|connection|socket|fetch)\b|\bECONN[A-Z]+\b/i.test(message)
    || /\b(?:other side closed|HTTP2 request did not get a response|WebSocket (?:error|closed unexpectedly))\b/i.test(message)
 ...
 *   to non-retryable `PI_AI_ERROR` failures. Generic `WebSocket error` failures
 *   map to `TRANSPORT` without overriding more specific provider error codes.
 */
