...
Set the Connection row's `config.authentication` to `none` only when external access control protects the entire Host, including WebSocket upgrades. This disables browser-session authentication, not the Host/Origin checks. Connection creates no launch token, reads or writes no browser-session credential record, and issues no cookie; `credentials` remains a required injected service. Launch URLs contain only the root path, without query or fragment. A legacy `GET /?token=...` redirects to `/` with `Cache-Control: no-store` and `Referrer-Policy: no-referrer`, without setting a cookie; other methods and index paths do not perform this cleanup. Existing browser cookies and stored signing secrets are left untouched. See the [external-access-control decision](../../../.agents/notes/implemented/feature/2026-09-17-connection-no-auth.md).

In both modes, every index and API request first passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403; in browser-token mode, a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` remains unsupported. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
...
**Runtime invariant:** No companion is published. Browser-session verification uses the signing secret loaded at activation without a second mutable credential cache, while the credentials companion owns record commit-event lifetime. Stream/reconnect sequencing and rpcId round-trip discipline are exercised directly by behavior specs, and route register/dispose symmetry is audited by the webserver companion.
