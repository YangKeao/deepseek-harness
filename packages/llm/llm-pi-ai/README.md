...
Generic `WebSocket error` failures carry `TRANSPORT`, which the normal retry policy accepts. More specific authentication, quota, invalid-request, and context-overflow errors keep their own codes; other unknown provider failures remain `PI_AI_ERROR` and are not retried by default. This classification does not change the configured streaming transport.
